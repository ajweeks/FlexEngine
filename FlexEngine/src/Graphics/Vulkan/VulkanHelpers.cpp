...
#undef STR
		...
		std::vector<u8> AsyncVulkanShaderCompiler::CompileSPIRVFromGLSL(EShLanguage stage, const std::string& sourceCode, glslang::EShSource language, std::string& outError)
		{
			std::vector<u8> ret;

			assert(language == glslang::EShSource::EShSourceGlsl);
			...
			const char* cs = sourceCode.c_str();
			...
			ret.resize(SpirV.size() * sizeof(u32));
			memcpy(ret.data(), SpirV.data(), SpirV.size() * sizeof(u32));

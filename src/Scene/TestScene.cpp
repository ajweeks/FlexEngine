...
void TestScene::Initialize(const GameContext& gameContext)
{
	...
	//m_UVSphere.GetTransform().position = vec3(-2.0f, 1.0f, 0.0f);

	////m_IcoSphere.LoadPrefabShape(gameContext, MeshPrefab::PrefabShape::ICOSPHERE);
	////m_IcoSphere.SetTransform({ vec3(2.0f, 1.0f, 0.0f), quat(vec3(0.0f)), vec3(0.5f, 0.5f, 0.5f) });

	//m_Cube.LoadPrefabShape(gameContext, MeshPrefab::PrefabShape::CUBE);
	//m_Cube.SetTransform({ vec3(-4.0f, 0.5f, 0.0f), quat(vec3(0.0f, 1.0f, 0.0f)), vec3(3.0f, 1.0f, 1.0f) });

	//m_Cube2.LoadPrefabShape(gameContext, MeshPrefab::PrefabShape::CUBE);
	//m_Cube2.SetTransform({ vec3(7.0f, 2.5f, 0.0f), quat(vec3(0.2f, 0.3f, 2.0f)), vec3(1.0f, 5.0f, 1.0f) });
	...
	//	{
	//		m_Cubes.push_back(MeshPrefab());
	//		MeshPrefab& cube = m_Cubes.back();
	...
	//		cube.SetTransform({ offset + vec3(i * cubeOffset, 0.0f, j * cubeOffset),
	...
	m_Teapot1.LoadFromFile(gameContext, "resources/models/cube.fbx");
	//m_Teapot1.LoadFromFile(gameContext, "resources/models/teapot.fbx");

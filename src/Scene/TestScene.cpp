...
void TestScene::Initialize(const GameContext& gameContext)
{
	...
	m_Scene->LoadFromFile(gameContext, "resources/models/scene_01_merged.fbx");
	m_Scene->SetTransform({
		glm::vec3(0.0f, -3.35f, 0.0f),
		glm::quat(glm::vec3(-glm::half_pi<float>(), 0.0f, 0.0f)),
		glm::vec3(1.0f) });
	AddChild(m_Scene);
	...
	//m_Landscape->SetTransform({
	//	glm::vec3(0.0f, -41.0f, 500.0f),
	//	glm::quat(glm::vec3(-glm::half_pi<float>(), 0.0f, 0.0f)),
	//	glm::vec3(1.0f) });
